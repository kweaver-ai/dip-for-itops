...
const handleReponseError = (response: any): Promise<any> => {
  ...
  if (!isNoHint) {
    const description = needMsg ? response.data.message : response.data.description;
    const detailCode = ERROR_CODE.includes(code.split('.')[1]);

    notification.error({
      title: description,
      description: detailCode ? code.split('').join('­­') : '' // json('')不是空，是一个英文换行的链接符，在vscode中显示不出来
    });
  }
...
export default handleReponseError;
